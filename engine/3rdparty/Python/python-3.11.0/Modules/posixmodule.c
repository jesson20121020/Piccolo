...
#  include <sys/mpctl.h>
    ...
    defined(__FreeBSD__)   || \
    defined(__NetBSD__)    || \
    defined(__APPLE__)) && !defined(__ORBIS__)
...
#endif
#ifdef __ORBIS__
        result = sceKernelStat(path->narrow, &stat);
        if (!result)
            result = ((stat.st_mode & mode) == mode);
#else
...
_posix_listdir(path_t *path, PyObject *list)
{
    ...
    Py_BEGIN_ALLOW_THREADS
    dirp = sceKernelOpen(name, SCE_KERNEL_O_RDONLY | SCE_KERNEL_O_DIRECTORY, 0644);
    Py_END_ALLOW_THREADS

    if (dirp < 0 || sceKernelFstat(dirp, &st) != 0 || st.st_blksize == 0) {
        list = path_error(path);
        goto exit;
    }
    if ((list = PyList_New(0)) == NULL) {
    ...
    buffer = (char*)malloc(st.st_blksize);
    if (!buffer) {
        goto exit;
    }
    Py_BEGIN_ALLOW_THREADS
    len = sceKernelGetdents(dirp, buffer, st.st_blksize);
    Py_END_ALLOW_THREADS
    if (len < 0) {
        goto exit;
    }
...
#elif defined(__ORBIS__)
    UTIME_TO_TIMEVAL;
    return sceKernelUtimes(path, time);
#else
    UTIME_TO_TIME_T;
    return utime(path, time);
#endif
...
ScandirIterator_iternext(ScandirIterator *iterator)
{
    ...
    while (1) {
        ...
        Py_BEGIN_ALLOW_THREADS
#ifdef __ORBIS__
        if (iterator->current < iterator->buffer_size) {
            direntp = (SceKernelDirent*)(iterator->buffer + iterator->current);
            iterator->current += (int)direntp->d_reclen;
        }
        else
            direntp = NULL;
#else
...
/*[clinic end generated code: output=6eb2668b675ca89e input=6bdd312708fc3bb0]*/
{
    ...
    iterator->dirp = -1;
    iterator->buffer = NULL;
    iterator->buffer_size = iterator->current = 0;
#else
    iterator->dirp = NULL;
#endif
...
#endif
    {
        ...
        Py_BEGIN_ALLOW_THREADS
#ifdef __ORBIS__
        iterator->dirp = sceKernelOpen(path_str, SCE_KERNEL_O_RDONLY | SCE_KERNEL_O_DIRECTORY, 0644);
        if (iterator->dirp >= 0) {
            if (sceKernelFstat(iterator->dirp, &st) == 0 && st.st_blksize) {
                iterator->buffer_size = (int)st.st_blksize;
                iterator->buffer = (char*)malloc(iterator->buffer_size);
            }
        }
#else
