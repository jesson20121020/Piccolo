...
# elif defined(__sun) || defined(__sgi) || defined(__ORBIS__)
...
setipaddr(const char *name, struct sockaddr *addr_ret, size_t addr_ret_size, int af)
{
    ...
    if (af == AF_INET || af == AF_UNSPEC) {
        ...
        memset(sin, 0, sizeof(*sin));
#ifdef __ORBIS__
        if (sceNetInetPton(SCE_NET_AF_INET, name, &(sin->sin_addr.s_addr)) < 0) {
...
#ifdef HAVE_GETSERVBYNAME //#if !defined(__ORBIS__) && !defined(__NX__)
...
#ifdef __ORBIS__
        if (sceNetInetPton(SCE_NET_AF_INET, ip_addr, &packed_addr) < 0)
          packed_addr = INADDR_NONE;
#else
        SUPPRESS_DEPRECATED_CALL
...
socket_inet_ntoa(PyObject *self, PyObject *args)
{
    ...
    PyBuffer_Release(&packed_ip);
#ifdef __ORBIS__
    retval = sceNetInetNtop(SCE_NET_AF_INET, &packed_addr, ip, sizeof(ip));
    if (!retval) {
        PyErr_SetFromErrno(socket_herror);
        return NULL;
    } else {
